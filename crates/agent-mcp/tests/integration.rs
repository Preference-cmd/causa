...
use reimagine_agent_mcp::McpToolSource;
...
fn echo_static_tool() -> Arc<dyn reimagine_context_kernel::Tool> {
    struct Echo;
    #[async_trait]
    impl reimagine_context_kernel::Tool for Echo {
...
async fn executor_dispatch_routes_dynamic_calls_and_maps_errors() {
    ...
    let outcome = executor
        .execute_with_limits(
            ToolCallPayload {
                call_id: reimagine_context_kernel::ToolCallId("call-2".into()),
    ...
    let outcome = executor
        .execute_with_limits(
            ToolCallPayload {
                call_id: reimagine_context_kernel::ToolCallId("call-4".into()),
    ...
    let outcome = executor
        .execute_with_limits(
            ToolCallPayload {
                call_id: reimagine_context_kernel::ToolCallId("call-5".into()),
    ...
    async fn invoke(
        ...
        if guard.is_empty() {
            return Err(ModelInvokeError::new(
                reimagine_context_kernel::ModelInvokeErrorKind::Permanent,
                "script exhausted",
            ));
        }
        Ok(guard.remove(0))
...
async fn kernel_turn_completes_through_the_mcp_source() {
    ...
    ctx.append_input(reimagine_context_kernel::TextPayload::new("hi"), "user")
        .unwrap();
    ...
    let result_block = out
        ...
        .find_map(|b| match &b.content {
            reimagine_context_kernel::BlockContent::ToolResult(r) => Some(r.clone()),
            _ => None,
        })
...
async fn http_transport_lists_and_invokes() {
    ...
    let call = ToolCallPayload {
        call_id: reimagine_context_kernel::ToolCallId("call-h1".into()),
