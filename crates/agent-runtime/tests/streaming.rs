...
async fn streaming_run_completes_and_observes_text_deltas() {
    ...
    let runner = TurnRunner::new(
        gateway.clone(),
        Arc::new(reimagine_agent_runtime::ToolExecutor::from_vec(vec![])),
    );
    ...
    assert!(matches!(
        &blocks[1].content,
        reimagine_context_kernel::BlockContent::Text(reimagine_context_kernel::TextPayload(t))
            if t == "hello world"
    ));
    ...
    match &events[0].kind {
        reimagine_agent_runtime::ContextEventKind::TextDelta { round_id, delta } => {
...
async fn streaming_tool_round_drives_next_round() {
    ...
    let rounds: Vec<RoundId> = events
        ...
        .filter_map(|e| match &e.kind {
            reimagine_agent_runtime::ContextEventKind::TextDelta { round_id, .. } => {
                Some(*round_id)
            }
            _ => None,
        })
...
impl ModelGateway for PendAfterFirstDelta {
    async fn invoke(
        &self,
        _req: &ModelRequest,
        _ctrl: &reimagine_context_kernel::AttemptControl,
    ) -> Result<reimagine_context_kernel::ModelOutput, ModelInvokeError> {
...
async fn cancellation_mid_stream_interrupts_explicitly() {
    ...
    #[async_trait::async_trait]
    impl reimagine_context_kernel::TurnInteraction for CancelOnFirstDelta {
    ...
    match out.result {
        TurnResult::Interrupted {
            cause: reimagine_agent_runtime::TurnInterruption::ExplicitCancellation,
        } => {}
...
async fn project_streaming_turn_interleaves_deltas_with_dispatches() {
    ...
    let kinds: Vec<&str> = events
        .iter()
        .map(|e| match &e.kind {
            reimagine_agent_runtime::ContextEventKind::TurnStarted => "started",
            reimagine_agent_runtime::ContextEventKind::TextDelta { .. } => "text",
            reimagine_agent_runtime::ContextEventKind::ToolCallDelta { .. } => "call_delta",
            reimagine_agent_runtime::ContextEventKind::ToolBatchDispatched { .. } => "dispatch",
            reimagine_agent_runtime::ContextEventKind::TurnOutcome { .. } => "outcome",
            reimagine_agent_runtime::ContextEventKind::ReasoningDelta { .. } => "reasoning",
        })
        .collect();
    assert_eq!(
        kinds,
    ...
    match &events[4].kind {
        reimagine_agent_runtime::ContextEventKind::ToolBatchDispatched {
            round_id, calls, ..
        } => {
            assert_eq!(*round_id, RoundId(0));
