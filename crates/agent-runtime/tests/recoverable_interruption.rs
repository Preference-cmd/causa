...
fn text_facts(context: &TurnContext) -> Vec<String> {
    context
        ...
        .filter_map(|b| match &b.content {
            reimagine_context_kernel::BlockContent::Text(t) => Some(t.0.clone()),
            _ => None,
        })
        .collect()
}

fn tool_result_fact(context: &TurnContext) -> reimagine_context_kernel::ToolResultPayload {
    context
        .blocks()
        .iter()
...
async fn decide_batch_pause_leaves_the_turn_open() {
    ...
    assert!(out.context.blocks().iter().any(|b| matches!(
        b.content,
        reimagine_context_kernel::BlockContent::ToolCall(_)
    )));
    assert!(!out.context.blocks().iter().any(|b| matches!(
        b.content,
        reimagine_context_kernel::BlockContent::ToolResult(_)
    )));
...
async fn steering_resume_injects_and_continues() {
    ...
    assert!(!resumed.context.blocks().iter().any(|b| matches!(
        b.content,
        reimagine_context_kernel::BlockContent::ToolResult(_)
    )));
}
