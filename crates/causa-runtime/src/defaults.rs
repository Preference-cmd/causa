//! The driver's internal fallback opinions. Private module since Slice 13:
//! the noop port defaults were removed (no in-repo consumers, and a zero
//! counter is behaviorally distinct from no counter — see `ExecutionOptions`
//! docs), leaving only the token-estimate fallback the executor needs.
//!
...
/// heuristic's single home in library code (7.4) — do not inline a copy
/// elsewhere.
